...
This document describes the architecture of GRANULES, a minimal multi-agent orchestrator that coordinates work across isolated Claude Code CLI workers using the Model Context Protocol (MCP).

## Overview

GRANULES implements a **work-queue pattern** where discrete tasks (called "granules") are claimed by workers, executed in isolation, and completed with summaries for coordination. The system is designed to be self-building: the first worker plans, subsequent workers implement.
...
The Store is the single source of truth for all granules. It's an in-memory data structure with no external dependencies.

**Key Methods:**
- `createGranule(class, content)` - Creates a new granule with auto-incremented ID (G-1, G-2, ...)
...
**Granule Data Structure (`src/types.ts`):**
```typescript
interface Granule {
  id: string;           // "G-1", "G-2", auto-incremented
  class: GranuleClass;  // explore, plan, implement, test, review, consolidate, Implemented
...
Exposes granule management as MCP tools via HTTP. Uses the `@modelcontextprotocol/sdk` library.
...
- **Endpoints**:
  - `/` and `/mcp` - MCP tool calls
  - `/health` - Health check

### 3. Orchestrator (`src/orchestrator.ts`)

The orchestrator manages the overall workflow, worker spawning, and exit conditions.
...
1. **Startup**: Starts MCP server on port 3000
2. **Bootstrap**: Creates initial "plan" granule if none exist
3. **Main Loop** (every 5 seconds):
   - Release stale claims (>30 minutes old)
   - Clean up completed worker processes
   - Check for exit condition
   - Spawn up to 3 workers for unclaimed granules
   - Log current state
...
Spawns isolated Claude Code CLI instances as child processes with dedicated git worktrees.
...
1. Entry Point (index.ts)
   ...
   ├─► Create Orchestrator with exit condition callback
   └─► Start orchestrator

2. Orchestrator Startup
   │
   ├─► Start MCP HTTP server (port 3000)
   └─► Bootstrap: Create "plan" granule if empty
...
5. Worker Execution
   │
   ├─► Claim granule via MCP: claim_granule(workerId, granuleId)
   ├─► Perform work (implement/plan/test/etc.)
   ├─► Create follow-up granules via MCP: create_granule(class, content)
   └─► Complete granule via MCP: complete_granule(workerId, granuleId, summary)
